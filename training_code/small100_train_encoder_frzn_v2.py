...
def freeze_encoder_layers(model):
    ...
    # Check if the shared embedding layer's requires_grad is still True
    if model.model.shared.weight.requires_grad:
        print("The requires_grad attribute for the shared embedding layer is still True.")
    else:
        print("The requires_grad attribute for the shared embedding layer is not True.")

    # Return the entire model with frozen encoder layers
    return model
...
def count_trainable_parameters(model):
    ...
    print(f"Number of trainable parameters: {trainable_params_million:.2f}M")
